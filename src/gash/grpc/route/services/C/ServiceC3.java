...
public class ServiceC3 extends RouteServerImpl {
    public static void main(String[] args) throws Exception {
        String path = args[0];
        try {
            Properties conf = getConfiguration(new File(path));
            RouteServer.configure(conf);
            ServiceC3 service = new ServiceC3();
            service.start();
    ...
    public void request(Route request, StreamObserver<Route> responseObserver) {
        ...
        byte[] raw = "Request processed by  processed by Server C3 of CustomQueue C".getBytes();
    ...
    private void startHeartBeatProcess(){
        new Thread(()->{
            ...
            while(true) {
                try {
                    RouteClient routeClient = new RouteClient(4003, 4000);
                    Route r = routeClient.request(rtn);
    ...
    public void start() throws Exception {
        this.svr = ServerBuilder.forPort(RouteServer.getInstance().getServerPort()).addService(new ServiceC3()).build();
        System.out.println("-- starting server -----");
        System.out.println("Listening to the port  " + RouteServer.getInstance().getServerPort());
        this.startHeartBeatProcess();
        this.svr.start();
        Runtime.getRuntime().addShutdownHook(new Thread(ServiceC3.this::stop));
    }
