...
public class ServiceC2 extends RouteServerImpl {
    public static void main(String[] args) throws Exception {
        String path = args[0];
        try {
            Properties conf = getConfiguration(new File(path));
            RouteServer.configure(conf);
            ServiceC2 service = new ServiceC2();
            service.start();
    ...
    private void startHeartBeatProcess(){
        new Thread(()->{
            ...
            while(true) {
                try {
                    RouteClient routeClient = new RouteClient(4002, 4000);
                    Route r = routeClient.request(rtn);
    ...
    public void start() throws Exception {
        this.svr = ServerBuilder.forPort(RouteServer.getInstance().getServerPort()).addService(new ServiceC2()).build();
        System.out.println("-- starting server -----");
        System.out.println("Listening to the port  " + RouteServer.getInstance().getServerPort());
        this.startHeartBeatProcess();
        this.svr.start();
        Runtime.getRuntime().addShutdownHook(new Thread(ServiceC2.this::stop));
    }
