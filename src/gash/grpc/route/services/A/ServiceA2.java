...
public class ServiceA2 extends RouteServerImpl {
    public static void main(String[] args) throws Exception {
        String path = args[0];
        try {
            Properties conf = getConfiguration(new File(path));
            RouteServer.configure(conf);
            ServiceA2 service = new ServiceA2();
            service.start();
    ...
    public void request(Route request, StreamObserver<Route> responseObserver) {
        ...
        byte[] raw = "Request processed by  processed by Server A2 of CustomQueue A".getBytes();
    ...
    private void startHeartBeatProcess(){
        new Thread(()->{
            ...
            while(true) {
                try {
                    RouteClient routeClient = new RouteClient(2002, 2000);
                    Route r = routeClient.request(rtn);
    ...
    public void start() throws Exception {
        this.svr = ServerBuilder.forPort(RouteServer.getInstance().getServerPort()).addService(new ServiceA2()).build();
        System.out.println("-- starting server -----");
        System.out.println("Listening to the port  " + RouteServer.getInstance().getServerPort());
        this.startHeartBeatProcess();
        this.svr.start();
        Runtime.getRuntime().addShutdownHook(new Thread(ServiceA2.this::stop));
    }
