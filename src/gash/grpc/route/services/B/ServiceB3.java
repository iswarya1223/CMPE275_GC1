...
public class ServiceB3 extends RouteServerImpl {
    public static void main(String[] args) throws Exception {
        String path = args[0];
        try {
            Properties conf = getConfiguration(new File(path));
            RouteServer.configure(conf);
            ServiceB3 service = new ServiceB3();
            service.start();
    ...
    public void request(Route request, StreamObserver<Route> responseObserver) {
        ...
        byte[] raw = "Request processed by  processed by Server B3 of CustomQueue B".getBytes();
    ...
    private void startHeartBeatProcess(){
        new Thread(()->{
            ...
            while(true) {
                try {
                    RouteClient routeClient = new RouteClient(3003, 3000);
                    Route r = routeClient.request(rtn);
    ...
    public void start() throws Exception {
        this.svr = ServerBuilder.forPort(RouteServer.getInstance().getServerPort()).addService(new ServiceB3()).build();
        System.out.println("-- starting server -----");
        System.out.println("Listening to the port  " + RouteServer.getInstance().getServerPort());
        this.startHeartBeatProcess();
        this.svr.start();
        Runtime.getRuntime().addShutdownHook(new Thread(ServiceB3.this::stop));
    }
