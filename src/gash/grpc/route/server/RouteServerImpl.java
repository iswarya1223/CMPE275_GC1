...
public class RouteServerImpl extends RouteServiceImplBase {
	 ...
	 */
	protected static Properties getConfiguration(final File path) throws IOException {
		if (!path.exists())
	...
	protected ByteString process(route.Route msg) {
		...
		final String blank = "acknowledged";
	...
	protected void start() throws Exception {
		this.svr = ServerBuilder.forPort(RouteServer.getInstance().getServerPort()).addService(new RouteServerImpl())
				.build();
	...
	protected void blockUntilShutdown() throws Exception {
		/* TODO what clean up is required? */
		this.svr.awaitTermination();
	}
